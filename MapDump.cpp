...
int findparam(int argc,char *argv[], char *infile[], char *outdir, char *prefix,  int * statframe, int * bin_factor, int * vbin,float * sigma);
...
int main(int argc,char *argv[])
{
	...
	for(int f=0;f<num_infile;f++)
	{
		...
		readmap_header(const_cast<char*>(fn), header, &nsymbt, &c,&r,&s,&amin,&amax,&amean,&arms, &mode, &datasize);

		int arraysize=c*r*s;
		int slice_size=c*r;

								 //read one slice
		float **  array= ReadMap(const_cast<char*>(fn), nsymbt,c,r,s, mode);
		...
		if(statframe>0)
		{
			...
			int statcont=0;
								 //statframe is upper limit of the frames to quant
			for(int z=0;z<s && z<statframe;z++)
			{
		...
		for(int z=0;z<s-vbin+1;z++)
		{
			char  png_name[1024];
								 ...
								 //default sigma=6
			float highcut=amean+sigma*arms;
...
*/
								 //slice start from 0
float ** ReadMap(const char *fn, int nsymbt,int c, int r,int s ,int mode)
{
	...
	for(int z=0;z<s;z++)
	{
		array[z]    =new float [c*r];
								 //read one slice
		read_slice(const_cast<char*>(fn), array[z],nsymbt,c,r,z, mode);
	}
	return array;
}
...
int bin(float * array, float * array_bin,int x, int y, int bin_factor)
{
	...
	{
								 //each element in new array
		for (int j=0;j<x-x%bin_factor;j=j+bin_factor)
		{
			*(array_bin+idx)=0;
