...
#define PNG_DEBUG 3
#include <png.h>
//simply take infile,pngname,offset, length, min, max x, y from argument
//if min==-1 and max==-1, recalculate min max
//mode direct :min=0 max=355
//mode 6rms:min=-6rms max=6rms
//mode min max: directly get min max from arguments
//mode stat: send back min max
...
int readmap_header(const char *filename0, char *header, int *nsymbt, int *c, int *r, int *s , float *amin, float *amax, float *amean, float *arms)
{
  ...
  if (in.is_open()){
    ...
    printf("min max rms:   %8.3f   %8.3f   %8.3f   %8.3f\n",*amin,*amax,*amean,*arms);
